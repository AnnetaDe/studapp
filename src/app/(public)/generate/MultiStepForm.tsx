...
import { LevelEnum, type ITestRequest, type ITestResponse, type ITestSubmit } from '@/ui/testGenerated/test.types';
...
export const MultiStepForm = ({ user_id }: Props) => {
    ...
    const [generatedQuestions, setGeneratedQuestions] = useState<ITestGenerated>({ test_id: '', level: LevelEnum.EASY, subject: '', test_data: [], count: 0 });
    const [answers, setAnswers] = useState<number[]>([]);
    const [submittedAnswers, setSubmittedAnswers] = useState<ISubmitedAnswers>({ test_id: '', selected_answers: [], user_id: '' });
    const [testResults, setTestResults] = useState<ITestResults>({ correct: 0, incorrect: 0, score: 0 });
    ...
    const mutationGenerate = useMutation({
        ...
        onSuccess: data => {
            ...
            setSubmittedAnswers({ test_id, selected_answers: [], user_id: user_id || '' });
            setStep(2);
        },
    });
            ...
            {step === 3 && testResults && (
                <div className="max-w-lg mx-auto bg-white p-4 rounded-lg shadow-md">
                    ...
                    <button
                        onClick={() => {
                            setStep(1);
                            setGeneratedQuestions({ test_id: '', level: LevelEnum.EASY, subject: '', test_data: [], count: 0 });
